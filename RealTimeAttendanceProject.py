...
# loop to iterate each image frame one by one
while True:
    success, image = capture.read()
    #  reducing the image size one fifth (.20) to speed up as the process is real time
    small_image = cv2.resize(image, (0, 0), None, 0.20, 0.20)
    # converting the smaller image from bgr to rgb
    small_image = cv2.cvtColor(small_image, cv2.COLOR_BGR2RGB)

    # enabling to locate multiple faces and sending all the face locations for face encoding in next step
    ...
    # one by one it will grab one face location from faces current frame list...
    ...
    for encodeFace, faceLocation in zip(currentFrameEncodings, facesInCurrentFrame):
        ...
        # calculating each face distance and the lowest face distance probability is the best match
        faceDistance = face_recognition.face_distance(encodingKnownList, encodeFace)

        # list of face distance
        print(faceDistance)

        matchIndex = np.argmin(faceDistance)

        # bounding box creation and writing the name
        if matchings[matchIndex]:
            studentName = studentNames[matchIndex].upper()
            print(studentName)

            y1,x2,y2,x1 = faceLocation
            # multiplying by 5 as the small image was one fifth of actual image
